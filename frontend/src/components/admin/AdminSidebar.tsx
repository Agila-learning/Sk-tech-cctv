...
import { LayoutDashboard, Package, ShoppingBag, Users, Clock, Megaphone, Calendar, LogOut, Camera, Settings, Activity, Shield, MessageSquare, Star, Hammer, IndianRupee, Target, Layers, UserCheck } from 'lucide-react';
    ...
    <>
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-4">
                <item.icon className={`h-5 w-5 ${isActive ? 'text-white' : 'text-fg-dim group-hover:text-blue-500 transition-colors'}`} />
      ...
      <div className="pt-8 border-t border-border-subtle space-y-4">
        <div className="px-5 py-4 bg-bg-muted rounded-2xl border border-border-subtle flex items-center space-x-4">
           <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl flex items-center justify-center text-white font-black text-xs">AD</div>
           <div className="flex flex-col">
              <span className="text-xs font-black text-fg-primary uppercase tracking-wider">Super Admin</span>
              <span className="text-[10px] font-bold text-fg-muted">Root Access</span>
           </div>
           <Link href="/admin/settings">
             <Settings className="h-4 w-4 text-fg-dim hover:text-fg-primary transition-colors cursor-pointer ml-auto" />
           </Link>
        </div>
        <button 
          onClick={() => logout()}
