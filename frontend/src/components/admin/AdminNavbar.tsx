...
const AdminNavbar = () => {
  ...
  return (
    <nav className="hidden lg:flex sticky top-0 z-40 w-full h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-border-base px-10 items-center justify-between">
      ...
      <div className="flex items-center space-x-6">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-right">
            <p className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-tighter leading-none">{user?.name || 'Admin'}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">{user?.role || 'Super Admin'}</p>
          </div>
