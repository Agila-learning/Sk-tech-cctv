...
export default function CartScreen({ navigation }: any) {
  ...
  return (
    <View style={s.root}><StatusBar barStyle="light-content" backgroundColor={Colors.background} />
      ...
      <View style={s.footer}>
        <View style={s.totRow}><Text style={s.totL}>Total:</Text><Text style={s.totV}>₹{cartTotal.toLocaleString()}</Text></View>
        <Button title={loading ? "Processing..." : "Proceed to Pay"} onPress={() => setInvoiceVisible(true)} icon={<CreditCard color="#fff" size={16} />} size="lg" disabled={loading} />
      </View>
          ...
          <View style={s.modalContainer}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
              <View style={s.qrSection}>
                <Image source={{ uri: 'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=upi://pay?pa=sktechnology@ybl%26pn=SK%20Technology%26cu=INR' }} style={s.qrImg} />
                <Text style={s.qrT}>Scan to Pay via UPI</Text>
                <Text style={s.scanT}>sktechnology@ybl</Text>
              </View>
              ...
              <View style={s.payMethods}>
                <TouchableOpacity style={[s.payBtn, paymentMethod === 'upi' && s.payBtnAct]} onPress={() => setPaymentMethod('upi')}>
                  <Text style={[s.payBtnT, paymentMethod === 'upi' && s.payBtnActT]}>Pay via UPI</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[s.payBtn, paymentMethod === 'cod' && s.payBtnAct]} onPress={() => setPaymentMethod('cod')}>
                  <Text style={[s.payBtnT, paymentMethod === 'cod' && s.payBtnActT]}>Cash on Delivery</Text>
                </TouchableOpacity>
              </View>

              {paymentMethod === 'upi' && (
                <TouchableOpacity style={[s.verifyBtn, upiPaid && s.verifyBtnAct]} onPress={() => setUpiPaid(!upiPaid)}>
                  <Text style={[s.verifyBtnT, upiPaid && s.verifyBtnActT]}>{upiPaid ? "✓ Payment Verified" : "Confirm UPI Payment Made"}</Text>
                </TouchableOpacity>
              )}
              ...
              <Button 
                ...
                onPress={checkout} 
                size="lg" 
                disabled={loading || (paymentMethod === 'upi' && !upiPaid)} 
                style={{ marginTop: 20 }} 
              />
            </ScrollView>
          </View>
