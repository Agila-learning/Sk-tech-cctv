...
export default function ReportsScreen() {
  ...
  const handleExport = async (format: 'pdf' | 'excel') => {
    try {
      ...
      const url = `${API_URL}/admin/export?type=revenue&format=${format}`;
      const ext = format === 'excel' ? 'xlsx' : 'pdf';
      const fileUri = `${(FileSystem as any).documentDirectory}revenue_report.${ext}`;
    ...
    <View style={s.root}><StatusBar barStyle="light-content" backgroundColor={Colors.background} />
      <View style={s.hdr}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <Button title="PDF" icon={<Download color="#fff" size={16} />} size="sm" onPress={() => handleExport('pdf')} />
          <Button title="Excel" icon={<Download color="#fff" size={16} />} size="sm" onPress={() => handleExport('excel')} />
        </View>
      </View>
